...
2. Откройте `.env` и заполните своими данными:
...
`CHANNEL_ID` должен быть цифровым; формат `@username` может не работать.
`CHECK_INTERVAL` задаётся в секундах и по умолчанию равен `3600` (один час).
`REQUEST_TIMEOUT` применяется к запросам VK API и скачиванию фотографий.
`PHOTO_MAX_BYTES` ограничивает размер одного скачиваемого фото; по умолчанию
20 МБ.
...
- Добавьте созданного бота администратором Telegram-канала и разрешите ему
  публиковать сообщения.
- `CHANNEL_ID` – цифровой ID канала вида `-1001234567890`. Его можно получить
  через Telegram Bot API `getUpdates` после публикации тестового сообщения или
  с помощью информационного бота, которому вы доверяете.

**VK:**
- `VK_TOKEN` – создайте приложение для разработчиков VK и получите токен с
  доступом к чтению стены. Не передавайте токен сторонним сайтам и сервисам.
- `GROUP_ID` – Числовой ID источника. Для сообщества используйте отрицательное
  значение, например `-123456789`; для страницы пользователя — положительное.
...
3. Скачивает фотографии с VK на стороне бота
4. Отправляет текст и фотографии в Telegram через Bot API
...
- watchdog – отслеживание изменений файлов при разработке
